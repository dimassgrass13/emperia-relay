// Emperia Geo-Proxy Relay v2 · Oxylabs Web Scraper API
// render:html → bypasses bot detection
// follow_redirect:false → no redirect loops
...
const GEO_LOCATION = {
  AT:'Austria', CY:'Cyprus', DK:'Denmark', IT:'Italy', LT:'Lithuania',
  NO:'Norway',  PL:'Poland', PT:'Portugal', LV:'Latvia', HU:'Hungary',
  CH:'Switzerland', DE:'Germany', SE:'Sweden', GB:'United Kingdom',
  FR:'France',  GR:'Greece',
};
...
app.get('/health', (req, res) => {
  res.json({
    ok: true, version: 'v2', api: OXYLABS_API,
...
app.get('/check', requireSecret, async (req, res) => {
  ...
  if (!geoLocation) return res.status(400).json({ ok: false, error: `Unknown GEO: ${geo}` });
  ...
  try {
    ...
    try {
      oxyResp = await fetch(OXYLABS_API, {
        ...
        body: JSON.stringify({
          ...
          follow_redirect: false,     // get first hop — no redirect loops
          parse:           false,
    ...
    const chain = [{
      url, status: firstStatus, location: locationResolved, headers: displayHdrs,
      note: isRedirect ? 'First hop via Oxylabs Web Scraper API' : undefined,
    }];

    return res.json({
      ok: true, finalUrl, finalStatus: firstStatus,
      redirectChain: chain, redirectCount: isRedirect ? 1 : 0,
      loadTime: Date.now() - started, finalHeaders: displayHdrs, error: null,
    });

  } catch(e) {
    return res.json({
      ok: false, finalUrl: url, finalStatus: null,
      redirectChain: [], redirectCount: 0,
...
app.listen(PORT, () => {
  console.log(`Geo relay v2 on :${PORT}`);
